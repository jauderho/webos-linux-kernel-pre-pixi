...
int get_img(struct mdp_int_img *img, struct fb_info *info, unsigned long *start,
	    unsigned long *len)
{
	int ret = -1;
	unsigned long vs;
...
int blit(struct fb_info *info, struct mdp_blit_int_req *req,
...
int mdp_blit(struct fb_info *info, struct mdp_blit_int_req *req)
{
	int ret;
	...
	get_img(&req->bg, info, &bg_start, &bg_len);
	...
	enable_mdp_irq(DL0_ROI_DONE);
	ret = blit(info, req, src_start, src_len, dst_start, dst_len, bg_start, bg_len);
	if (ret)
		goto err_bad_blit;
	ret = mdp_ppp_wait();
	if (ret)
